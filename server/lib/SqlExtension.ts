...
type CaseValue<TReturn> = TReturn | SQL<TReturn> | SQL.Aliased<TReturn> | SQLWrapper;
class CaseBuilder<TReturn> implements SQLWrapper {
  private chunks: SQL[] = [];

  constructor(initialCondition: SQL, initialResult: CaseValue<TReturn>) {
   ...
   */
  whenThen(condition: SQL | undefined, result: CaseValue<TReturn>): this {
   ...
   */
  else(value: CaseValue<TReturn>): SQL<TReturn> {
...
export function caseWhen<TReturn>(condition: SQL, then: CaseValue<TReturn>) {
